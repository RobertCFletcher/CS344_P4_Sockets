...
**Description:  This is the client portion for encoding ciphertext, runs once when called to talk to server
** and gets an encoded message back from the server
***********************************************************************************************************/
...
void clientCheck(int socketFD) {
	...
	charsWritten = send(socketFD, "otp_enc", 7, 0);
