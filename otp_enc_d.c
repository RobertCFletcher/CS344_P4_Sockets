...
**Description:  This is the server portion for encoding ciphertext, runs constantly until stopped
***********************************************************************************************************/
...
void check_otp_enc(char* requested, int connection){
	...
	if (strcmp("otp_enc", requested) != 0) {
		//print out an error message
		fprintf(stderr, "ERROR, connection not coming from otp_enc");
...
void encryptMessage(int message_size, char* entireMessage, char* key) {
	...
	for (i = 0; i < message_size; i++) {
		...
		//perform the encryption of the character as per the instructions.  Add message and key, mod 27 (alphabet plus space)
		...
		entireMessage[i] = ((entireMessage[i] - 65 + key[i] - 65) % 27 + 65);
